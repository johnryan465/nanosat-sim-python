...
class Kinetics(PythonAttitudeProviderModifier):
    ...
    def getAttitude(self, pvProv: PVCoordinatesProvider, date: AbsoluteDate, frame: Frame) -> Attitude:
        ...
        if self.satellite.get_set_of_magnetorquers() is not None:
            ϵ_1 = self.satellite.get_inertia_inverse() @ (array(self.satellite.get_set_of_magnetorquers().state.get_control_torque()) + external_torque)

        ϵ_2 = (-1) * self.satellite.get_inertia_inverse() @ cross_product_matrix(ω) @ self.satellite.get_inertia() @ ω

        # reaction wheel
        ϵ_3 = array([0, 0, 0])
        ϵ_4 = array([0, 0, 0])
        if self.satellite.get_set_of_reaction_wheels() is None:
            ϵ_3 = -1 * self.satellite.get_inertia_inverse() @ cross_product_matrix(ω) @ array(
                self.satellite.get_set_of_reaction_wheels().state.get_angular_momentum(ω))
            ϵ_4 = -1 * self.satellite.get_inertia_inverse() @ array(self.satellite
